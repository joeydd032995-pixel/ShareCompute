...
from inference_pipeline_lib import (
 DEFAULT_ACTIVATION_BYTES, DEFAULT_HOST, DEFAULT_USABLE_GB,
 DISCOVERY_MULTICAST_GROUP, DISCOVERY_PORT, DISPLAY, INFER_SERVICE_ID, PLATFORMS,
 ActivationMsg, InferShard, activation_nbytes_for_budget, buffer_budget_bytes,
 connect_with_deadline, discover_infer_hub, fake_compute, make_prompt_activation,
...
def run_peer(
    ...
    *,
    ...
    print(
        f"p {DISPLAY[platform]} ({role}): listen {data_host}:{data_port}",
        flush=True
    )
    if hub_host is None or hub_port is None or hub_port <= 0:
        remaining = max(0.1, deadline - time.monotonic())
        print(
            f"p {DISPLAY[platform]}: wait {INFER_SERVICE_ID} beacon on "
            f"{DISCOVERY_MULTICAST_GROUP}:{discovery_port} (timeout {remaining:.1f}s)",
            flush=True
        )
        beacon = discover_infer_hub(remaining, discovery_port=discovery_port)
        if beacon is None:
            print(
                f"p {DISPLAY[platform]}: discovery fail — no {INFER_SERVICE_ID} beacon on "
        ...
        print(
            f"p {DISPLAY[platform]}: hub {hub_host}:{hub_port} "
            f"(epoch={beacon.epoch})",
            flush=True
        )
    else:
        print(
            f"p {DISPLAY[platform]}: hub {hub_host}:{hub_port} (no disc)",
            flush=True
        )
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if ack and ack.get("type") == "ack":
                ...
                print(
                    f"p {DISPLAY[platform]}: ack role={role} as {node_id} epoch={join_epoch}",
                    flush=True
                )
                ctrl = sock
                break
            if ack and ack.get("type") == "error":
                ...
                try:
                    ...
                    pass
                print(
                    f"p {DISPLAY[platform]}: join rejected ({ack.get('reason')})",
                    file=sys.stderr,
                    flush=True
                )
                data_sock.close()
        ...
        print(
            f"p {DISPLAY[platform]}: join fail {hub_host}:{hub_port} within "
    ...
    while time.monotonic() < plan_deadline:
        ...
        if msg.get("type") == "shutdown":
            print(f"p {DISPLAY[platform]}: shutdown before plan ({msg.get('reason')})")
            _cleanup(ctrl, data_sock)
            return 1
        if msg.get("type") == "error":
            print(f"p {DISPLAY[platform]}: hub error {msg}", file=sys.stderr)
            _cleanup(ctrl, data_sock)
            return 1
    if plan_msg is None:
        print(f"p {DISPLAY[platform]}: no plan", file=sys.stderr)
        _cleanup(ctrl, data_sock)
        return 1
    plan_epoch, token_count, shards = plan_dict_to_shards(plan_msg)
    if plan_epoch != join_epoch:
        print(
            f"p {DISPLAY[platform]}: REFUSE data — plan epoch {plan_epoch} "
    ...
    if my is None:
        print(f"p {DISPLAY[platform]}: not in plan", file=sys.stderr)
        _cleanup(ctrl, data_sock)
        return 1
    ...
    try:
        ...
        print(
            f"p {DISPLAY[platform]}: budget refuse — {exc}",
            file=sys.stderr,
            flush=True
        )
        try:
        ...
        return 1
    print(
        f"p {DISPLAY[platform]}: plan={plan_epoch} rank={my.rank} "
    ...
    try:
        if my.role == "frontend" or my.rank == 0:
            rc = _run_frontend(
                ctrl, buf, data_sock, shards, my, plan_epoch, token_count, prompt,
                pipe_deadline, buf_budget=buf_budget, act_nbytes=act_nbytes
            )
        else:
            rc = _run_worker(
                ctrl, buf, data_sock, shards, my, plan_epoch, token_count,
                pipe_deadline, buf_budget=buf_budget
            )
        return rc
    except Exception as exc:  # noqa: BLE001
        print(f"p {DISPLAY[platform]}: pipe exc: {exc}", file=sys.stderr)
        try:
        ...
        _cleanup(ctrl, data_sock)
def _run_frontend(
    ctrl: socket.socket, buf: bytearray, data_sock: socket.socket,
    shards: list[InferShard], my: InferShard, epoch: str, token_count: int,
    prompt: str, deadline: float, *, buf_budget: int, act_nbytes: int
) -> int:
    if len(shards) < 2:
    ...
    print(f"p fe: connected → rank {next_shard.rank} {next_shard.data_host}:{next_shard.data_port}", flush=True)
    ...
    try:
        for tid in range(token_count):
            ...
            if len(activation) > buf_budget:
                send_msg(ctrl, {"type": "pipeline-error", "reason": f"activation-exceeds-budget:{len(activation)}>{buf_budget}"})
                return 1
            seed = make_prompt_activation(f"{prompt}|tok{tid}", act_nbytes)
            msg = ActivationMsg(epoch=epoch, token_id=tid, rank_from=my.rank, rank_to=next_shard.rank, payload=activation)
            try:
            ...
            try:
                ...
                return 1
            if result is None:
                print(f"p fe: miss result {tid} (worker dead?)", file=sys.stderr, flush=True)
            ...
            if result.rank_from != last_shard.rank or result.rank_to != my.rank:
                print(f"p fe: rank mismatch (got {result.rank_from}→{result.rank_to}, expected {last_shard.rank}→{my.rank})", file=sys.stderr, flush=True)
                send_msg(ctrl, {"type": "pipeline-error", "reason": f"rank-mm:{result.rank_from}->{result.rank_to}!={last_shard.rank}->{my.rank}"})
                return 1
            completed += 1
            print(f"p fe: token {tid} complete (final from rank {result.rank_from}, {result.nbytes}B)", flush=True)
...
def _run_worker(
    ...
    while time.monotonic() < deadline and inbound is None:
        ...
        conn.setblocking(False)
        inbound = conn
        print(f"p wr {my.rank}: inbound {addr[0]}:{addr[1]}", flush=True)
    if inbound is None:
    ...
    print(f"p wr {my.rank}: connected → rank {dest.rank} {dest.data_host}:{dest.data_port}" + (" (to frontend)" if is_last else ""), flush=True)
    processed = 0
    try:
        while processed < token_count and time.monotonic() < deadline:
            ...
            try:
                act = recv_activation(inbound, min(deadline, time.monotonic() + 5.0), max_payload_bytes=buf_budget)
            except ValueError as exc:
                print(f"p wr {my.rank}: bad activation frame: {exc}", file=sys.stderr, flush=True)
                ...
                return 1
            if act is None:
                print(f"p wr {my.rank}: act timeout after {processed}/{token_count}", file=sys.stderr, flush=True)
            ...
            if act.rank_from != expected_from or act.rank_to != expected_to:
                print(f"p wr {my.rank}: rank mismatch (got {act.rank_from}→{act.rank_to}, expected {expected_from}→{expected_to})", file=sys.stderr, flush=True)
                send_msg(ctrl, {"type": "pipeline-error", "reason": f"rank-mm:{act.rank_from}->{act.rank_to}!={expected_from}->{expected_to}"})
                return 1
            out_payload = fake_compute(act.payload, my.layer_count, act.token_id, my.rank)
            if len(out_payload) > buf_budget:
                send_msg(ctrl, {"type": "pipeline-error", "reason": f"activation-exceeds-budget:{len(out_payload)}>{buf_budget}"})
                return 1
            out = ActivationMsg(epoch=epoch, token_id=act.token_id, rank_from=my.rank, rank_to=dest.rank, payload=out_payload)
            try:
                ...
                return 1
            processed += 1
            print(f"p wr {my.rank}: token {act.token_id} layers[{my.start_layer},{my.end_layer}) → rank {dest.rank}", flush=True)
        _wait_shutdown(ctrl, buf, deadline)
        return 0
    finally:
        try:
