...
Uses a distinct discovery service id (`sharecompute-infer`) so this protocol is not
confused with the four-platform pool demo (`sharecompute-pool`).
"""
...
INFER_SERVICE_ID = "sharecompute-infer"
...
def parse_infer_beacon(raw: bytes) -> Optional[HubBeacon]:
    """Parse a beacon; only accept INFER_SERVICE_ID (reject sharecompute-pool)."""
    try:
    ...
    if not isinstance(msg, dict):
        return None
    if msg.get("service") != INFER_SERVICE_ID:
        return None
    try:
...
def announce_infer_beacon(
    ...
    payload = encode_infer_beacon(hub_host, hub_port, epoch, platforms)
    sock.sendto(payload, (group, discovery_port))


def discover_infer_hub(
    timeout_s: float,
    discovery_port: int = DISCOVERY_PORT,
    group: str = DISCOVERY_MULTICAST_GROUP,
) -> Optional[HubBeacon]:
    """Listen for a sharecompute-infer beacon until timeout. Returns None on miss."""
    ...
    try:
        while True:
            ...
            try:
                ...
                continue
            beacon = parse_infer_beacon(data)
...
def plan_infer_shards(
    ...
    Rejects plans that cannot fit the model (+ overhead) in aggregate usable RAM,
    or where any shard's estimated_gb exceeds that peer's usable_gb.
    """
