#!/usr/bin/env python3
"""Control-plane hub for Phase A inference-pipeline simulation.
...
def run_hub(
    ...
    *,
    ...
    print("ShareCompute inference-pipeline sim (Phase A) — control-plane hub")
    print("=================================================================")
    print(f"Listening on {host}:{port}  timeout={timeout_s:.1f}s")
    print(f"Expecting platforms: {', '.join(sorted(expected_set))}")
    print(f"Frontend seat: {DISPLAY[frontend_platform]}  tokens={token_count}")
    if enable_beacon:
        print(
            f"Beacon: multicast {DISCOVERY_MULTICAST_GROUP}:{discovery_port} "
            f"service={INFER_SERVICE_ID} epoch={epoch}",
            flush=True,
        )
    else:
    ...
    def handle_client(conn: socket.socket, addr) -> None:
        ...
        try:
            ...
            if data_port <= 0:
                ...
                return
            peer = InferPeer(platform, node_id, usable_gb, role, data_host, data_port)
            with lock:
                # One seat per platform for this demo.
                connected[platform] = peer
                client_conns[platform] = (conn, buf)
                print(
                    f"  ✓ {DISPLAY[platform]} JOIN role={role} node={node_id} "
    ...
    try:
        ...
        for s in shards:
            print(
                f"  rank {s.rank}  {DISPLAY[s.platform].ljust(7)}  role={s.role.ljust(8)}  "
