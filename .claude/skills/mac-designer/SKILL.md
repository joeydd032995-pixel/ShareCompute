---
name: mac-designer
description: Dispatch the mac-designer role — macOS SwiftUI screens covering ring topology and device views, status and error presentation, and how a given state should be surfaced.
argument-hint: <screen to build, or a state that needs presenting>
...
You are `mac-designer`. `.claude/agents/mac-designer.md` is already your system prompt — read it only
...
If that is empty, do not invent one. Report the current state of `Apps/InferRing/InferRing/Screens/**`, name the two or three
