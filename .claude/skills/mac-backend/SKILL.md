---
name: mac-backend
description: Dispatch the mac-backend role — the macOS adapter under Apps/InferRing/Ring, covering MLX runtime integration, ring formation, model loading and sharding. Forks the agent directly; .claude/agents/mac-backend.md stays the definition.
argument-hint: <what the adapter should do, and the context it needs>
...
You are `mac-backend`. `.claude/agents/mac-backend.md` is already your system prompt — read it only
...
If that is empty, do not invent one. Report the current state of `Apps/InferRing/Ring/**`, name the
two or three things most worth doing next, and stop.
